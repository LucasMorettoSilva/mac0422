...
/*EP3########################################################################*/
  ...
  /* worst-fit */
    ...
    do {
      prev_ptr = biggest_prev = NIL_HOLE;
      hp = biggest_h = hole_head;
      while (hp != NIL_HOLE && hp->h_base < swap_base) {
        /* always in search of the biggest hole-length */ 
        if (hp->h_len > biggest_h->h_len) {
          biggest_h = hp;
  ...
  /* best fit */
    ...
    do {
      ...
      while (hp != NIL_HOLE && hp->h_base < swap_base) {
      /* enquanto existir uma lacuna e o comeco das lacunas nao ultrapassarem swap_base: */
      /* percorre a lista toda em busca da melhor lacuna */
       if (hp->h_len >= clicks)
      /*a lacuna possui o comprimento minimo*/
       if(best_hp == NIL_HOLE || (hp->h_len < best_hp->h_len)){
      /*caso ainda não tenhamos um candidato, atualiza */
      /*OU se já tivermos um canditato porém pior do que a lacuna analisada*/
       prev_best = prev_ptr;
       best_hp = hp;
       }
       prev_ptr = hp;
      ...
      /* uma vez percorrida a lista em busca da melhor lacuna, atualiza a lista e retorna o
      endereço*/
      if(best_hp != NIL_HOLE){
      /* We found a hole that is good enough. Use it. */
       old_base = best_hp->h_base; /* remember where it started */
       best_hp->h_base += clicks; /* bite a piece off */
       best_hp->h_len -= clicks; /* ditto */
      /* Remember new high watermark of used memory. */
       if(best_hp->h_base > high_watermark)
         high_watermark = best_hp->h_base;

      /* Delete the hole if used up completely. */
       if (best_hp->h_len == 0) del_slot(prev_best, best_hp);
  ...
  /* next fit */
  else if (alloc_mode == NEXT_FIT) {
    do {
      prev_ptr = NIL_HOLE;

      if (next_fit_head == NIL_HOLE) {
        next_fit_head = hole_head;
      }
...
int do_memalloc() {
  ...
  int type, p_end, p_idx;
      
  /* check if caller process is root */
  p_end = m_in.m_source;
  for (p_idx = 0; p_idx < NR_PROCS; p_idx++)
    if (mproc[p_idx].mp_endpoint == p_end)
      break;
  
  /* caller no longer exists or is not root righ now */
  if (p_idx == NR_PROCS || mproc[p_idx].mp_effuid != ((uid_t) 0))
    return(EINVAL);

  /* change memory allocation mode according with passed */
  type = m_in.m1_i1;
